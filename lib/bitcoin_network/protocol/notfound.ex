defmodule BitcoinNetwork.Protocol.NotFound do
  alias BitcoinNetwork.Protocol.{NotFound, InvVect, VarInt}
  ...
  def parse(binary) do
    with {:ok, count, rest} <- parse_count(binary),
      ...
      {:ok, %NotFound{count: count, inventory: inventory}, rest}
    end
  end
...
defimpl BitcoinNetwork.Protocol, for: BitcoinNetwork.Protocol.NotFound do
